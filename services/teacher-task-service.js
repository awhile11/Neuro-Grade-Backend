  ...
  // 2️⃣ Fetch test questions by ID
  async function fetchTest(testId) {
    const docRef = db.collection("tests").doc(testId.toString());
    ...
    container.innerHTML = `<h2>${testData.title}</h2>`;
    testData.questions.forEach((q, idx) => {
      container.innerHTML += `<p>${idx + 1}. ${q.text} (Max Marks: ${q.max_marks})</p>`;
    });
  }

  // Call with a test ID (from Django response)
  fetchTest(1);

//javascript for dynamic rendering of test questions and answers
