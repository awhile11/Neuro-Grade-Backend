  ...
  //     const testTitleInput = document.getElementById('testSearch'); // Assume there's an input for searching tests
  ...
  //     try {
  //       const response = await fetch(`http://127.0.0.1:8000/api/generate-test/?count=${count}`, {
  //         method: 'POST',
  //         headers: { 'Content-Type': 'application/json' },
  //         body: JSON.stringify({ count, difficulty, types })
          
  //       });

  //       console.log('Requesting AI questions — payload:', { count, difficulty, types });
  ...
  //         questionCount++;
  //         console.log(questionCount, q.content);
  ...
  //   questionsList.addEventListener('input', e => { if (e.target.tagName === 'TEXTAREA') { const idx = Array.from(questionsList.querySelectorAll('textarea')).indexOf(e.target); questionsState[idx] = e.target.value; localStorage.setItem('questionsState', JSON.stringify(questionsState)); } });
  ...
  //             const response = await fetch('http://127.0.0.1:8000/api/save-test/', {
  //                 method: 'POST',
  //                 headers: { 'Content-Type': 'application/json' },
  //                 body: JSON.stringify({
  //                     name: testName,
  ...
  //         document.getElementById('test-id').value = test.id;  // Store the test ID in the form
  //         document.querySelector('sma').textContent = 'Currently Editing';  // Change <sma> text
  //     }
  //     async function getTestFromDatabase(testId) {
  //     try {
  //         const testRef = doc(db, "tests", testId); // Firestore reference to the test document
